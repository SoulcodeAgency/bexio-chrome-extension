...
npm resolves this on its own: TS 7 installs **nested** under each package (`packages/*/node_modules/typescript`), and a JS-based TS (5.x or 6.0.x — whatever `typescript-eslint`'s peer range currently allows; 6.0.3 at the time of writing) is auto-installed at the **root** to satisfy that peer requirement. So `npm run typecheck` runs on 7 while `npm run lint` parses on the root copy. The root copy is not declared in any `package.json` — it tracks whatever `typescript-eslint` peers on, and is pinned by the lockfile. Do not add an explicit root `typescript` dependency; that would need its own Dependabot hold to stop it being bumped to 7 and breaking the linter.

Bumping `typescript-eslint` by hand trips over this split: a plain `npm i` re-places the new `@typescript-eslint/*` nodes, tries the workspaces' TS 7 as the candidate for their `typescript` peer and fails with `ERESOLVE` — even when the peer range did not change. Dependabot's lockfile updater passes `--force`, which is why its PRs never hit it. Do the same, and only for that step: install every other bump first (plain `npm i --workspaces --include-workspace-root`), then `npm i -w @bexio-chrome-extension/side-panel-import -D @typescript-eslint/parser@<v> @typescript-eslint/eslint-plugin@<v> --force`. Afterwards check `npm ls typescript`: the root copy must still sit inside the peer range (this is how it moved from 5.9.3 to 6.0.3) and the three nested copies must still be 7.x, and `npm run lint` must pass.
