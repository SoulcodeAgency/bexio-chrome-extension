...
A Chrome Manifest V3 extension that augments bexio's time tracking pages (`office.bexio.com`): save/load form data as templates, import time entries from ManicTime via a browser side panel, replace bexio's tooltip icons with inline text, and make bexio's date columns sort newest-first. Published on the Chrome Web Store.
...
**Where the version shows up.** On chrome://extensions, and on the page itself: the injected Templates block's `h2` carries it in its `title` (hover "Templates"), and the side panel prints it in the footer line. Both read `chrome.runtime.getManifest().version` — the manifest is the only file `build:test` stamps, so `package.json` would have printed the release version and never said which test build was on the page. Handy when chrome://extensions is not in reach, e.g. when someone reports a bug from a page you are looking at.
...
- **Content scripts** are declared in `packages/chrome-extension/public/manifest.json`, keyed by bexio URL:
  ...
  - `monitoring/list`, `monitoring/list/*`, `pr_project/listMonitoring/*`, `pr_project/showPackage/*`, `kb_invoice/show/id/*` → `src/apps/bexioProjectList/index.ts` — the tooltip→text replacement (`convertPopover`) and the date column sorting (`utils/dateSort.ts`), both driven by `MutationObserver`s because bexio re-renders tables. `onTableRendered()` is the one callback for the initial render and every observer. Sorting: the link of every _unsorted_ date column is rewritten from `/o/asc` to `/o/desc` (bexio's delegated `.ajxl` handler reads `href` at click time, so that is all it takes), and the opt-in "Newest first" toggle (`autoDateSortSetting`, default off, all four pages) clicks the `monitoring.DATE` link of every time entry list that arrives unsorted — which they practically always do: bexio's menu opens the time tracking list through `resetListView/1`, a project's and a work package's "Zeiten" tab forget their sort on every page load, the invoice's "Zeiten importieren" modal on every opening. The loop guard is per list block, not per page load (the modal has to be sorted on each opening): a block is clicked once, and a list whose answer comes back unsorted is given up on. "Sorted" means a `span.caret` inside a **header** link; the rows have carets of their own.
...
- `docs/architecture/list-sorting.md` — how bexio's sort links, its delegated click handler and its three header states work, where the sort state lives and why the menu loses it (`resetListView`), the date link rewrite, the "Newest first" toggle with its one-click-per-page-load guard, known limits.
...
- Do not edit `manifest.json`'s `version` by hand — it's generated by `updateManifest.js` during release. The `description` in `package.json` is likewise touched by tooling. The "last update" date the extension shows is not a file at all — it is stamped at build time (`scripts/build-date/buildDate.ts`, Vite `define` as `__BUILD_DATE__`), because only a build happens on every release path.
...
- **Land every PR with `--subject` and an empty `--body`**, or it lands in `CHANGELOG.md` twice. `release-please` parses every commit since the last release, and the merge commit carries the message a second time. There is no repository setting that avoids this — GitHub allows only three title/message combinations, and each one leaves `release-please` a conventional commit to parse (see `docs/architecture/publishing.md`). So spell the merge message out:

  ```bash
  gh pr merge <N> --merge --subject "Merge pull request #<N> from SoulcodeAgency/<branch>" --body ""
  ```

  That subject is not a conventional commit and the body is empty, so nothing is counted twice — verified on #154, whose merge commit `c14b2c4` produced no changelog line while its branch commit `ab3538e` did. Squash merges do not have the problem either — `release-please` reads GitHub's `(#N)` squash subject as one commit — but they collapse the branch's SHAs, which is what the rule above exists to prevent.
