...
export async function remove(id, key = defaultKey): Promise<any> {
    ...
    const filteredEntries = await chrome.storage.local.get(key).then((result) => {
        if (result[key] && Array.isArray(result[key])) {
            return result[key].filter(entry => entry.id !== id);
        }
        return [];
    });
...
export async function save(entries, key = defaultKey): Promise<any> {
